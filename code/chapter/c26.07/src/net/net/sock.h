 ...
 * 为什么 listen 和 accept 也放在这里？UDP 根本不用啊？
 * 统一定义“socket 可能支持的能力”，具体某个协议支不支持，交给实现决定。
 */
...
/*
特性,使用 .h + .c（标准做法）,直接 include .c（错误做法）
链接,独立编译，通过链接器组合，无冲突,极易引发“重复定义”错误
速度,支持增量编译，速度快,修改一处，全量重编，速度极慢
依赖,依赖关系清晰,依赖关系混乱，形成代码“面条”
分发,可以只分发头文件和二进制库,必须暴露所有源代码*/




//sock_init：初始化一个 socket（设置协议、ops、等待器等） sock_uninit：释放 socket 内部资源（但不一定销毁对象）
net_err_t sock_init(sock_t* sock, int family, int protocol, const sock_ops_t * ops);
void sock_uninit (sock_t * sock);
//接收上层 API 请求 → 转成内部处理（消息驱动入口）
...
//初始化 socket 子系统   建立 socket 表 / 资源池 / 初始化环境
