/**
 * @brief 套接字接口类型
把用户调用（x_socket / x_send / x_recv）
↓
转换成内部 sock + req + TCP/UDP 调用
 */
#ifndef NET_SOCKET_H
#define NET_SOCKET_H
...
//系统已经有这些宏，为什么还要自己定义？#undef 再 #define的意思是？
/*为了“完全脱离系统 socket，实现自己的协议栈”*/
#undef AF_INET
 ...
 * //sockaddr 和 sockaddr_in 大小都是 16字节 API只认 sockaddr*
 * 通用接口（sockaddr）
        ↓
具体实现（sockaddr_in）
 */
//socket API 要支持多种协议（IPv4 / IPv6 / 其他） 需要统一的地址接口 通用接口
struct x_sockaddr {
 ...
 * @brief 套接字结构
 */
//_socket_t是用户层 socket → 内部 sock 的桥
