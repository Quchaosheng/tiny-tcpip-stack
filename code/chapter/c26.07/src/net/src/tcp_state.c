 ...
 * 在 TCP 连接的 CLOSED 状态下，如果接收到的数据报不是 RST 包，则发送一个 RST 包，以 强制关闭连接
 */
 ...
 * 该函数用于 ESTABLISHED 状态下，处理接收到的 TCP 报文。
RST 位检查：如果收到 RST 包，说明连接需要被重置，执行连接重置操作。
SYN 位检查：如果收到 SYN 包，说明可能是重新连接，发送 RST 并关闭连接。
ACK 处理：确认已发送的数据，通过 tcp_ack_process 来处理 ACK，确认数据已经收到。
数据处理：通过 tcp_data_in 处理接收到的数据。
发送数据：调用 tcp_out_event 尝试发送数据，并根据需要更新窗口。
进入 CLOSE_WAIT 状态：如果收到 FIN 包，且所有数据都已接收，则进入 CLOSE_WAIT 状态。
 */
 ...
 * 该函数用于 CLOSE_WAIT 状态下，处理接收到的 TCP 报文。
RST 位检查：如果收到 RST 包，表示连接异常，需要重置连接。
SYN 位检查：如果收到 SYN 包，表示连接错误，需要发送 RST 并重置连接。
ACK 处理：通过 tcp_ack_process 处理已确认的数据，确保数据已经被接收。
继续发送数据：即使在 CLOSE_WAIT 状态，依然可以发送数据并更新窗口。
 */
 ...
 * 该函数用于 LAST_ACK 状态下，处理接收到的 TCP 报文。
RST 位检查：如果收到 RST 包，连接需要被重置。
SYN 位检查：如果收到 SYN 包，表示连接错误，需要发送 RST 并重置连接。
ACK 处理：处理收到的 ACK 包，确保确认数据已经发送。
继续发送数据：虽然在 LAST_ACK 状态，但如果有数据未发送，继续发送。
终止连接：当所有数据（包括 FIN）发送完并确认后，调用 tcp_abort 关闭连接。
 */
 ...
 * LISTEN 状态：服务器在等待客户端的连接请求。
检查 RST 位：如果收到 RST 包，说明连接被强制重置，但在监听状态下收到 RST 并不需要做处理，因此返回 NET_ERR_OK。
收到 ACK 包：如果收到 ACK 包，说明客户端在发送连接请求后又进行了某种响应，这在 LISTEN 状态下是不正常的，因此会发送 RST 包并返回错误。
处理 SYN 包：
如果收到 SYN 包，表示客户端正在请求连接。
队列满检查：首先检查 backlog 队列是否已满，如果已满，则返回 NET_ERR_FULL。
创建子进程：如果有足够的资源，则创建一个新的 TCP 子进程来处理该连接（通过 tcp_create_child）。
响应 SYN：为新创建的子进程发送 SYN 包，并进入 SYN_RECVD 状态，等待客户端确认。
 */
 ...
 * SYN_RECVD 状态：表示服务器已经收到连接请求并发出了 SYN-ACK，现在等待客户端的 ACK 确认。
处理 RST 包：如果收到 RST 包：
如果是由 被动打开 生成的连接，则直接关闭连接。
如果是 主动打开 且同时打开，表示连接被拒绝，结束连接并中止数据传输。
处理 SYN 包：如果收到 SYN 包，表示对方请求重新连接，发送 RST 并重置连接。
ACK 处理：通过 tcp_ack_process 处理客户端的 ACK，确认数据是否已经接收。
处理 FIN 包：如果收到 FIN 包，表示对方关闭了连接，进入 CLOSE_WAIT 状态。
进入 ESTABLISHED 状态：如果连接正常建立，则进入 ESTABLISHED 状态，表示连接成功建立并可以开始数据传输。
发送 ACK 包：尝试发送 ACK 包以响应客户端。
 */
