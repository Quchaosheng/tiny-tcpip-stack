...
static pktblk_t* pktblock_alloc(void) {
    // 不等待分配，因为会在中断中调用
    //加锁必须在访问共享资源之前，解锁必须在之后，这样才能保证线程安全
    nlocker_lock(&locker);
    ...
    while (size) {
        ...
        if (!new_block) {
            ...
            if (first_block) {
                // 失败，要回收释放整个链
                nlocker_lock(&locker);
                //从 first_block 开始，把后面串起来的所有 block 全部释放掉
        ...
        int curr_size = 0;
        if (add_front) {
            //从 size 和 PKTBUF_BLK_SIZE 里选一个较小的，赋值给 curr_size
...
/**
 * 为数据包新增一个头   
 * 在包前面增加头部空间，优先复用首块预留区；如果不够，就按是否要求连续来决定是否新分配 block。
 */
...
//释放后续 block 只删掉完整块，tail_blk 内部可能仍有多余空间，需要单独调整 size，使 total_size 精确等于 to_size
        // 减掉后续所有块链中的容量
 ...
 */
//把 src 的所有块（pktblk）连接到 dest 的末尾，然后释放 src
...
net_err_t pktbuf_set_cont(pktbuf_t* buf, int size) {
    ...
    // 先将第一个blk的数据往前挪，以在尾部腾出size空间       注意：这里是“搬数据”，不是新分配 block
#if 0
...
net_err_t pktbuf_init(void) {
    ...
    //block_list：pktblk 内存池管理 单个数据块 (pktblk_t)  每个 pktblk 存放 一段实际数据（payload） 用于 pktbuf 内的数据存储
    //pktbuf_list：pktbuf 内存池管理 pktbuf 对象 (pktbuf_t)链表、游标、大小等元数据  每个 pktbuf 是 链式缓冲区管理器  不存实际 payload，而是管理 pktblk
...
static void move_forward(pktbuf_t* buf, int size) {
    ...
    // 全局位置记录从 pktbuf 开始到当前位置一共读/写了多少字节  方便函数知道总共操作了多少数据
    buf->pos += size;
    //块内部位置记录在当前 block (curr_blk) 内的读/写偏移  直接指向下一个读/写字节
    buf->blk_offset += size;
 ...
 * @param size 最多读取的字节量
 */
//从 src 写入 size 字节到 pktbuf，从当前游标位置 curr_blk + blk_offset 开始，自动跨 block。
int pktbuf_read(pktbuf_t* buf, uint8_t* dest, int size) {
    ...
    // 循环读取所有量
    //在链式缓冲区（pktbuf）里顺序写入或读取数据，自动处理跨块（pktblk）情况
    while (size > 0) {
        int blk_size = curr_blk_remain(buf);
        //当前循环本次操作的字节数：如果当前 block 剩余空间小于剩余数据 → 只处理 block 剩余空间
        //否则处理剩余全部数据       保证不会超出 block 边界
        int curr_copy = size > blk_size ? blk_size : size;
 ...
 * 从当前位置开始计算校验和
 * @param rw 读取器
 * @param size 计算多大区域的校验和       保证网络数据在传输或存储过程中没有被篡改或损坏
 * @return 16位校验     IPv4、TCP、UDP 校验和在协议标准里就是 16 位
 */
...
/*             整体逻辑流程图
               +------------------+
                ...
                +--------+---------+
                         ...
                         v
                +--------------------------+
                | pktblock_alloc_list(size)|
                | 按需分配一个或多个 block  |
                +--------+-----------------+
                         |
                         v
                +--------------------------+
                | pktbuf_insert_blk_list   |
                | 挂到 buf 的 block 链表上 |
                +--------+-----------------+
                         |
                         v
       ...
       |                  对 buf 进行操作                 |
       |--------------------------------------------------|
       | add_header / remove_header / resize / join       |
       | set_cont / seek / read / write / copy / fill     |
       | checksum16                                       |
       +----------------------+---------------------------+
                              |
                              v
...
值得mark的点：
1.为什么用“块链表”而不是“大数组”
这个项目没有直接用单块大缓冲区保存网络包，而是把一个包拆成多个 block 链接起来。
这样可以按需分配空间，减少浪费，也更适合协议栈里频繁加包头、删包头、拼包的场景。
