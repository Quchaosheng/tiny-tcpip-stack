...
net_err_t udp_bind(sock_t* sock, const struct x_sockaddr* addr, x_socklen_t len) {
    ...
    // 不允许重复绑定端口号  一个 socket 只能有一个“本地身份”
...
net_err_t udp_connect(sock_t* sock, const struct x_sockaddr* addr, x_socklen_t len) {
    //UDP connect 只是：把 remote_ip 和 remote_port 写到 socket 里  
    //UDP connect 的作用：作用A：发送时不用每次都传目标地址   作用B：接收时只接受这个远端的数据
...
net_err_t udp_sendto (struct _sock_t * sock, const void* buf, size_t len, int flags, const struct x_sockaddr* dest,
    ...
    // 本地可能没有绑定端口，此时需要分配一个（自动分配）
    //为什么必须要有本地端口   因为 UDP 头里必须有源端口和目的端口
    //而且未来对方回包时，要根据目标端口把包送回来。所以本地端口不能为空。
    ...
    // 数据拷贝过去
    //把应用层传进来的 payload 拷贝到包缓冲区
    //UDP 特点：应用写多少，就是一个完整的 UDP 数据报
    ...
    // 通过UDP层发送出去
    //udp_sendto = socket语义处理 + 参数准备   udp_out= UDP报文构造 + 交给IP层
...
/**
 * @brief 读取一些数据
 * 此函数流程1. 从 recv_list 取出一个包  2. 把来源地址填给 src  3. 把数据拷贝给用户 buf  4. 释放这个包
 */
net_err_t udp_recvfrom(sock_t* sock, void* buf, size_t len, int flags,
    ...
    //拿到 UDP 的私有状态 通用 socket 字段在 sock_t  UDP 专属字段（比如 recv_list）在 udp_t
...
//因为 recv_list 链表里挂的节点本质上是 pktbuf_t.node，
//所以这里要通过 nlist_entry 从链表节点恢复出整个 pktbuf_t
...
/*socket 地址结构中的端口字段通常按网络字节序表示
内核/协议栈内部保存时可能是主机字节序
返回给上层时要转回网络序*/
    // 重新定位     移除自定义的 udp_from_t 头
    /*接收队列里的 pktbuf 布局是：
+------------------+
| udp_from_t       |
+------------------+
...
+------------------+*/
    ...
    //如果用户缓冲区 buf 很小，就只能拷贝一部分  如果用户缓冲区足够大，就拷贝整个报文内容
...
/**
 * @brief 关闭udp连接
 * UDP 没有连接状态 关闭比较简单，直接移除，不需要再通信
 */
//把它从系统里摘掉   把它占用的接收缓存释放  把对象内存还回去
net_err_t udp_close(sock_t* sock) {
    udp_t * udp = (udp_t *)sock;

    // 把 socket 从全局 udp_list 摘掉
    //一旦 remove 之后：后续来的 UDP 包不会再匹配到这个 socket
    //这个 socket 相当于从协议栈“注销”了
...
sock_t* udp_create(int family, int protocol) {
    ...
    // 分配一个raw sock结构
    //UDP socket 不是 malloc 动态分配，而是从固定对象池里分配
    ...
    //新建出来的 UDP socket，必须插入到全局表里，才能被未来收到的 UDP 包匹配到。
...
static net_err_t is_pkt_ok(udp_pkt_t * pkt, int size) {
    //至少要有 UDP 头和实际接收长度不能小于 UDP 头里声明的长度
...
//此时传进来的 buf 还不是“纯 UDP 包”，而是：IPv4 层交过来的整个 IP 数据包
    ...
    // 填充来源信息
    //去掉 UDP 头的大部分，只留下前面一小段空间，把它重解释成 udp_from_t
    ...
    // 写到接收队列，通知应用程序取
    //每收到一个 UDP 报文，就把它作为一个独立消息插入该 socket 的 recv_list
    if (nlist_count(&udp->recv_list) < UDP_MAX_RECV) {
        nlist_insert_last(&udp->recv_list, &buf->node);

        // 信号量通知线程去取包。如果需要由DNS处理，则将由DNS       DNS:域名 → IP地址
...
net_err_t udp_out(ipaddr_t * dest, uint16_t dport, ipaddr_t * src, uint16_t sport, pktbuf_t * buf) {
    ...
    // 发送前，添加UDP头部          [payload]变为[UDP头 | payload]
    ...
    // 数据层转换并计算伪校验和      要对伪首部（源 IP 目的 IP 协议号 UDP 长度）校验
...
/*UPD模块：
1 接收UDP包 → 找到对应socket → 交给应用
udp_bind()
udp_connect()
alloc_port()
udp_find()
2 应用发送数据 → 构造UDP包 → 交给IP层
udp_sendto()
udp_out()
udp_in()
recvfrom()
