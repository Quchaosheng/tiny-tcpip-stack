...
static net_timer_t cache_timer;                       // arp表扫描定时器  定期扫描ARP表
static arp_entry_t cache_tbl[ARP_CACHE_SIZE];    // arp缓存
static mblock_t cache_mblock;                // 空闲arp分配结构
static nlist_t cache_list;                   // 动态表 保存：正在使用的ARP表项
...
void display_arp_entry(arp_entry_t* entry) {
    plat_printf("%d: ", (int)(entry - cache_tbl));       // 序号 entry在ARP表里的下标
...
/**
 * @brief 释放entry中所有等待的数据包 可能存在情况：IP包等待ARP解析
 */
 ...
 * 把一条 ARP 表项的信息全部写进去，并初始化超时和重试参数。
 */
static void cache_entry_set(arp_entry_t* entry, const uint8_t* hwaddr,
    ...
    /*ARP表项有两种重要状态
    NET_ARP_RESOLVED  已解析
    NET_ARP_WAITING   等待解析*/
    entry->state = state;
    //记录所属网卡
    entry->netif = netif;
 ...
 * ARP表是怎么建立、更新、触发发送数据包
 */
//cache_insert()根据收到的 IP + MAC 更新 ARP 表  如果表里没有就新建  如果已有就更新
/*| 参数       | 含义       |
| -------- | -------- |
| netif    | 网络接口     |
| pro_addr | IP地址     |
...
/**
 * @brief 使用IP包来更新ARP表   ARP 表不一定只靠 ARP 包更新，IP 包也可以更新 ARP 表
 */
void arp_update_from_ipbuf(netif_t* netif, pktbuf_t* pkt) {
    // 单个buf肯定要至少容纳IP包
    //协议栈的 pktbuf 可能是分段存储的    保证：Ethernet + IP header 在连续内存中
    ...
    //解析 IP 头 IP header = Ethernet header 后面的位置
...
/**
 * @brief ARP定时处理
 * 减掉每个表项的tmo，一旦发现超时，则删除 处理超时ARP表项
 * 应对两种问题：
...
static void arp_cache_tmo(net_timer_t* timer, void * arg) {
    ...
    for (curr = cache_list.first; curr; curr = next) {
        ...
        changed_cnt++;
        switch (entry->state) {
            //ARP解析成功但时间太久了 可能导致IP对应的MAC已经变了
            case NET_ARP_RESOLVED: {
                // 切换至Pending状态，发送ARP请求
 ...
 */
//如果这个网卡要关闭、注销，和它相关的 ARP 记录就没意义了，必须清掉
...
void arp_clear(netif_t* netif) {
    ...
    for (node = nlist_first(&cache_list); node; node = node->next) {
        ...
        if (e->netif == netif) {
            //这个表项里可能还有：等待ARP解析完成后再发送的数据包 包现在已经没法通过这个网卡发了，所以必须释放
            cache_clear_all(e);
            nlist_remove(&cache_list, node);
...
net_err_t arp_init(void) {
    ...
    // 创建定时器后启动 
    //注册一个周期性定时器，定期调用 arp_cache_tmo() NET_TIMER_RELOAD表示是一个周期重复触发的定时器
...
/**
 * @brief 向指定网口上发送ARP查询请求1分配ARP数据包  2填充ARP请求内容  3发送以太网广播
 */
//发送ARP请求：“谁是这个IP？请告诉我你的MAC地址”
...
| ---------------------- | --------- |
| netif                  | 从哪个网卡发送   |
| NET_PROTOCOL_ARP       | 以太网类型=ARP |
| ether_broadcast_addr() | 目标MAC=广播  |
| buf                    | ARP数据     |
*/
 ...
 */
//Gratuitous ARP通常在 网卡刚启动时发送   目的：1 提前更新别人ARP表 2检测 IP 冲突
...
2 等待回复
3 IP包先存起来
这些等待发送的包存到 entry->buf_list
当ARP解析成功：cache_send_all()把所有等待的包发送出去
...
ARP等待队列  当MAC还没解析时IP包不能丢  所以要挂到ARP entry里的 buf_list
