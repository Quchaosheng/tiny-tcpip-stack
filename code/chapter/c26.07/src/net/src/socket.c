...
/**
 * @brief 关闭socket
 * 关闭套接字的操作不仅仅是简单地释放资源，还涉及到处理套接字关闭后的状态（比如等待连接断开、关闭连接等）。
 */
 ...
 * 使用 sock_req_t 结构体填充套接字请求信息，并通过 exmsg_func_exec 调用发送函数。
每次请求发送时检查 req.wait 标志，如果需要等待，则使用 sock_wait_enter 来等待数据写入缓存。
 */
ssize_t x_send(int sockfd, const void* buf, size_t len, int flags) {
    ...
    while (len) {
        ...
        // 等待数据写入发送缓存中。注意，只是写入缓存，并不一定发送完成
...
/**
 * @brief 接收数据
 * 使用 sock_req_t 结构体填充请求并通过 exmsg_func_exec 调用接收函数。
如果数据没有被接收（req.data.comp_len == 0），则通过 sock_wait_enter 等待数据的到达。
如果连接被远端关闭，则返回 0，表示接收完成。
 */
 ...
 * @brief 连接到远程主机
 * 检查传入的地址参数是否有效。
使用 sock_req_t 结构体填充连接请求的信息，并通过 exmsg_func_exec 调用连接函数。
如果连接过程中需要等待，则调用 sock_wait_enter 等待连接成功。
 */
...
//sa_family：这是套接字地址结构中的一个字段，表示地址族。AF_INET 表示该地址是一个 IPv4 地址。
    ...
    // 地址和端口号不可能为空
    //sin_addr.s_addr：这是 x_sockaddr_in 结构体中的字段，表示IPv4地址。
    //INADDR_ANY 表示该地址为 "任何地址"（即 0.0.0.0）。如果地址是 INADDR_ANY，则该地址不合法，无法建立连接。
 ...
 * @brief 发送数据包到指定的目的地
 * 检查目标地址和数据长度是否有效。
使用 sock_req_t 填充请求结构，并通过 exmsg_func_exec 调用发送数据包的函数。
如果需要等待数据写入缓存，则使用 sock_wait_enter 等待。
 */
ssize_t x_sendto(int sockfd, const void* buf, size_t size, int flags, const struct x_sockaddr* dest, x_socklen_t addr_len) {
    ...
    //family 是套接字地址结构中的一个字段，用于指定地址的类型。常见的值有：AF_INET：表示IPv4地址。AF_INET6：表示IPv6地址。AF_UNIX：表示Unix本地通信地址。
...
//size剩余待发送的字节数    send_size 会包含总共发送的字节数 start更新数据缓冲区的起始位置
...
/**
 * @brief 接收数据包消息处理
 * 该函数通过 exmsg_func_exec 向内核层请求接收数据包。
如果接收到数据，则将数据填充到 buf 中，并返回接收到的数据字节数。
如果没有数据，需要等待连接或者重试，直到接收到数据或发生错误。
 */
...
//plat_strlen(name) 返回字符串 name 的长度，不包括字符串结尾的空字符 '\0'
//hostent_extra_t 是一个自定义结构体，它存储了与 DNS 查询相关的附加信息
    ...
    //调用 exmsg_func_exec 函数发送 DNS 请求，这会异步处理域名查询。如果发送失败，返回错误。
...
//检查 DNS 请求是否成功。如果失败，返回错误。
    if (dns_req->err < 0) {
        dbg_error(DBG_SOCKET, "dns resolve failed.");
        *err = dns_req->err;
        goto dns_req_error;
    }
    // 将查询结果填充到 hostent_extra_t 结构体中。
    //将 IP 地址从 DNS 请求的结果中提取并填充到缓存区中。
 ...
 * 主要步骤：
发起监听请求：
创建 sock_req_t 请求结构体，并填充相关信息，如 sockfd（套接字文件描述符）和 backlog（最大连接队列大小）。
通过 exmsg_func_exec 调用处理函数 sock_listen_req_in 来发起监听请求
 */
