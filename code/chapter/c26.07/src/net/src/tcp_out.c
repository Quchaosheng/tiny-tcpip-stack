...
net_err_t tcp_send_reset(tcp_seg_t * seg) {
    ...
    //RST 是 TCP 用来立即终止连接或拒绝连接的控制报文 当一方发现连接有问题，或者根本不存在这个连接时，就会发送 RST。
...
net_err_t tcp_send_ack(tcp_t* tcp, tcp_seg_t * seg) {
    ...
    //发送方序号 seq 用我下一次要发的数据序号。 确认号 ack 用我下一次想收到的对方数据序号
    out->seq = tcp->snd.nxt;
    out->ack = tcp->rcv.nxt;
    out->flags = 0;
    out->f_ack = 1;                             // ACK标志
...
/**
 * @brief 发送Keepalive报文 TCP 连接长时间没数据时，会发 keepalive
 */
net_err_t tcp_send_keepalive(tcp_t* tcp) {
   // 分配一个TCP包
    ...
    out->sport = tcp->base.local_port;
    out->dport = tcp->base.remote_port;
    out->seq = tcp->snd.nxt - 1;        // 要求序号为对方期望的序号-1   要“骗对方回 ACK”，从而判断连接还活着
    out->ack = tcp->rcv.nxt;
    out->flags = 0;
    out->f_ack = 1;
    out->win = (uint16_t)tcp_rcv_window(tcp);
...
/**
 * @brief 针对一个已经找到的 TCP 连接，主动发一个 RST 包  
 * tcp_send_reset(seg) 是 根据收到的报文来回 RST。
tcp_send_reset_for_tcp(tcp) 是 手里已经有 tcp 连接对象了，直接构造一个 RST 发出去
 */
 ...
 */
//如果要发 SYN，就顺便把 MSS 选项写进 TCP 头后面
...
static void get_send_info (tcp_t * tcp, int rexmit, int * doff, int * dlen) {
    if (rexmit) {
        ...
        *dlen = tcp_buf_cnt(&tcp->snd.buf) - *doff;
        ...
        //snd.una：最早未确认的  snd.nxt：下一个准备发送的
...
net_err_t tcp_transmit(tcp_t * tcp) {
    ...
    //总序号长度 = 数据长度 + SYN + FIN SYN 和 FIN 也各占 1 个序号
    int seq_len = dlen;
    ...
    hdr->ack = tcp->rcv.nxt;
    hdr->flags = 0;
    //第一次建连接时，SYN 包里顺便告诉对方我的 MSS
    hdr->f_syn = tcp->flags.syn_out;   // 复位完后不清理，因为可能要重传  除了第一次发 SYN 之外，后面的 TCP 包一般都要带 ACK。
    if (hdr->f_syn) {
    ...
    //有发送缓冲区已经空了，FIN 才能真正发出去     TCP 要求：先把数据发完，再发 FIN。
...
net_err_t tcp_retransmit(tcp_t* tcp) {
   ...
   if (hdr->f_syn) {
    ...
    //如果这次重发顺便把一些“之前还没正式发到”的新字节也发出去了，才推进 snd.nxt
...
net_err_t tcp_ack_process (tcp_t * tcp, tcp_seg_t * seg) {
    ...
    //una 是 未确认的数据序号       nxt 是 下一个待发送的序号，它表示 期望发送的下一个字节的序号
    //tcp_hdr->ack 是对方确认的序号，tcp->snd.una 是自己最早未确认的序号
    ...
    if (curr_acked > 0) {
        ...
        //如果有数据被确认（curr_acked > 0），并且 fin_out 标志位为 1，表示 FIN 包已经发送，并且 FIN 也已被确认。
 ...
 * 注意有可能不能一次性全部写入数据
 */
/*这段代码将应用层传入的数据写入到发送缓冲区中。
free_cnt 是剩余空间，wr_len 是实际写入的数据长度。
如果缓冲区没有足够的空间，数据不会被写入。*/
...
static void tcp_out_timer_tmo (struct _net_timer_t* timer, void * arg) {
    ...
    switch (tcp->snd.ostate) {
        case TCP_OSTATE_SENDING: {
            // 如果处于发送状态超时，就会调用重传函数 tcp_retransmit 来重发未确认的数据包。
            //然后，重传的超时时间会进行 指数回退，即每次重传的超时值会翻倍，直到达到最大值
            ...
            //在网络出现丢包或超时时，自动重传数据，并逐渐增加重传超时，直到重传成功或达到最大重传次数。
            tcp->snd.rexmit_cnt = 1;
            //rto 是重传超时时间，表示等待对方确认数据的最大时间。 <<= 1 是将 rto 翻倍，表示每次重传超时时间都增加
            //指数退避算法：每次超时后，将重传超时（rto）翻倍      避免过快的重传导致网络拥塞
            tcp->snd.rto <<= 1;
            //ostate表示当前发送的状态。将其设置为 TCP_OSTATE_REXMIT 表示进入 重传状态
        ...
        case TCP_OSTATE_REXMIT: {
            ...
            /*忽略重传 RTT：在计算超时时，忽略重传包的 RTT 计算，避免错误地降低 RTO，确保超时时间计算的准确性。
            二进制指数退避：每次超时重传，RTO 会根据重传次数进行 指数退避，逐步加长重传间隔，防止过多重传干扰网络*/
 ...
 * @brief 更改输出状态
 * 根据当前状态决定是否启动定时器。
如果状态是 TCP_OSTATE_IDLE，则移除定时器，因为不需要超时处理。
如果状态是 TCP_OSTATE_SENDING 或 TCP_OSTATE_REXMIT，则启动重传定时器
 */
void tcp_set_ostate (tcp_t * tcp, tcp_ostate_t state) {
 ...
 /**
 * @brief 空闲状态下的事件
 * 处理空闲状态下，接收到 SEND 事件 时，表示可以开始发送数据，调用 tcp_transmit 开始发送数据。
然后将状态转换为 TCP_OSTATE_SENDING，表示当前正在发送数据
 */
...
/**
 * @brief 发送状态处理
 * 在 发送状态下，接收到 SEND 事件 时，检查是否所有数据都已确认。
如果数据已发送完毕或者 FIN 标志位已设置（即数据已完全发送），则检查是否还有数据需要发送。
如果还有数据要发送，继续发送并保持在 发送状态。
如果没有数据要发送，进入 空闲状态。
 */
...
/**
 * @brief 重传处理
 * 在重传状态下，接收到 SEND 事件 时，检查是否所有数据都已确认。
如果数据确认并且没有未确认的数据（或者 FIN 包已经发送），则继续发送并返回 发送状态。
如果数据尚未确认，进入 重传状态，并通过 tcp_retransmit 函数重新发送丢失的数据。
 */
static void tcp_ostate_rexmit_in (tcp_t * tcp, tcp_oevent_t event) {
    switch (event) {
        case TCP_OEVENT_SEND: {
            ...
            if ((tcp->snd.una == tcp->snd.nxt) || tcp->flags.fin_out) {
                if (tcp_buf_cnt(&tcp->snd.buf) || tcp->flags.fin_out) {
                    // 有数据要发送或正在发送 FIN 包 发送前需要检查窗口，如果为0则进入坚持状态，否则会进入普通发送状态
                    tcp_transmit(tcp);
                    tcp_set_ostate(tcp, TCP_OSTATE_SENDING);
                } else {
                    tcp_set_ostate(tcp, TCP_OSTATE_IDLE);
...
/**
 * @brief 向外输出数据
 * 根据 当前的发送状态 ostate 和 接收到的事件 event，
 * 调用相应的处理函数（tcp_ostate_idle_in、tcp_ostate_sending_in、tcp_ostate_rexmit_in）来处理不同的状态和事件。
 */
...
//根据当前的状态 tcp->snd.ostate，从 state_fun 数组中取出对应的函数指针，然后调用相应的函数处理事件。
    // 执行回调函数
