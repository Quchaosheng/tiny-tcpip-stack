 ...
 * @brief 显示定时器列表信息
 */
/*调试模式开启（宏条件成立） → 有真正的函数，打印定时器列表
调试模式关闭（宏条件不成立） → display_timer_list() 什么也不做
不会无条件执行，是否生成和执行取决于宏 DBG_DISP_ENABLED(DBG_TIMER)*/
...
#else
#define display_timer_list()//display_timer_list不写的话
                            //当 DBG_DISP_ENABLED(DBG_TIMER) 为 0（即调试关闭）时：编译器会报 “未定义的标识符” 或 “函数未声明” 错误
#endif

/**
 * @brief 将结点按超时时间从小到达插入进链表中
 */
//将新的定时器插入 timer_list，保持剩余时间递增
/*差值调整：
插入定时器时，会调整 insert->curr 和后继定时器 curr，保证链表总时间一致
比如插入中间节点：后继定时器的 curr 减去插入节点的时间

排序逻辑：
insert->curr > curr->curr → 继续往后找
insert->curr < curr->curr → 插入当前节点前，并调整差值
...
static void insert_timer(net_timer_t * insert) {
    ...
    nlist_for_each(node, &timer_list) {
        ...
        // 待插入的结点超时比当前结点超时大（待插入定时器的剩余时间比链表当前节点剩余时间长），应当继续往后寻找
...
| -------- | ---------- | ------------- |
| `name`   | 定时器名字      | 用于打印调试        |
| `reload` | 周期时间(ms)   | 对于周期定时器，触发后重载 |
| `curr`   | 当前剩余时间(ms) | 倒计时触发回调       |
| `proc`   | 回调函数指针     | 定时器到期调用的函数    |
| `arg`    | 回调参数       | 用户数据，回调可用     |
| `flags`  | 定时器标志      | 控制周期性等行为      |
*/
    plat_strncpy(timer->name, name, TIMER_NAME_SIZE);
    //末尾手动加 '\0' 确保字符串安全结束
    timer->name[TIMER_NAME_SIZE - 1] = '\0';
    timer->reload = ms;
    //对于周期性定时器，触发一次后会用 reload 重新装载 curr
    timer->curr = timer->reload;
    timer->proc = proc;
    timer->arg = arg;
    timer->flags = flags;
...
/**
 * @brief 定时处理模块初始化：保证链表为空，防止未初始化就插入定时器
调试输出可以确认初始化完成
 */
...
/**
 * @brief 取消定时器：从链表中删除指定定时器
   调整后继定时器的 curr，保证时间连续性
 */
...
/*功能：实现 软定时器（Software Timer）
用途：
协议栈内部使用（例如 UDP/TCP 超时重传、ARP 超时）
定时任务处理（例如周期性检查、事件触发）
特点：
链表管理：所有定时器保存在单链表 timer_list
支持周期重载：可重复触发的定时器
软定时器：不依赖硬件定时器，每次调用 net_timer_check_tmo(diff_ms) 更新状态
...
1差值链表：
curr = 相对于前一个定时器的剩余时间
插入/删除/检查都需要调整后继节点 curr
2周期定时器：
NET_TIMER_RELOAD 标记
回调执行后重载重新插入链表
3软定时器：
net_timer_check_tmo(diff_ms) 驱动
不依赖硬件定时器，可灵活处理任意时间间隔
4链表稳定性：
遍历时用临时 next 保存下一节点
避免删除或插入时破坏遍历
5调试：
display_timer_list() 用于观察链表状态
输出 curr/reload/flags 理解差值链表行为
