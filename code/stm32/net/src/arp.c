...
static net_err_t is_pkt_ok(arp_pkt_t* arp_packet, uint16_t size, netif_t* netif) {
    ...
    // 做一些检单的ip地址检查
    ipaddr_t src_ip;
    ipaddr_from_buf(&src_ip, arp_packet->send_paddr);
    if (ipaddr_is_any(&src_ip) || ipaddr_is_equal(&src_ip, &netif->ipaddr)
        || ipaddr_is_direct_broadcast(&src_ip, &netif->netmask)
        || ipaddr_is_local_broadcast(&src_ip)) {
        dbg_dump_ip(DBG_ARP, "src ip invalid", &src_ip);
        return NET_ERR_NOT_SUPPORT;
    }
...
net_err_t arp_in(netif_t* netif, pktbuf_t* buf) {
    ...
    if (ipaddr_is_equal(&target_ip, &netif->ipaddr)) {
        ...
        err = cache_insert(netif, arp_packet->send_paddr, arp_packet->send_haddr, 0);
        if (err < 0) {
            return err;
        }
    }
...
net_err_t arp_resolve(netif_t* netif, const ipaddr_t* ipaddr, pktbuf_t* buf) {
    ...
    if (entry) {
        ...
        // 插入arp列表
        if (nlist_count(&entry->buf_list) >= ARP_MAX_PKT_WAIT) {
            dbg_warning(DBG_ARP, "too many packet waiting. ignore this packet");
            return NET_ERR_FULL;
        }
