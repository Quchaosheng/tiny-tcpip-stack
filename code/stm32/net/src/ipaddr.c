...
int ipaddr_is_match(const ipaddr_t* dest, const ipaddr_t* src, const ipaddr_t * netmask) {
    ...
    ipaddr_t dest_hosid = ipaddr_get_host(dest, netmask);
    ipaddr_t src_hostid = ipaddr_get_host(src, netmask);

    // 本地为全0地址，匹配
    if (ipaddr_is_any(dest)) {
        return 1;
    }

    // 源全1地址：本地受限广播，匹配
    if (ipaddr_is_local_broadcast(src)) {
        return 1;
    }

    // 二者：同一子网广播，匹配
    if (ipaddr_is_direct_broadcast(src, netmask) && ipaddr_is_equal(&dest_netid, &src_netid)) {
        return 1;
    }

    // 本网络，同一主机，即网络号为0，主机号相同
    if (ipaddr_is_any(&src_netid) && ipaddr_is_equal(&dest_hosid, &src_hostid)) {
        return 1;
    }
