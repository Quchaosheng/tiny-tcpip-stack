...
#define UDP_NR      UDP_MAX_NR + 1          // 算上DNS
static udp_t udp_tbl[UDP_NR];           // UDP控制块数组
...
net_err_t udp_init(void) {
    ...
    // 应用端可能有多任务访问，因此需要锁
    mblock_init(&udp_mblock, udp_tbl, sizeof(udp_t), UDP_NR, NLOCKER_NONE);
    nlist_init(&udp_list);
...
net_err_t udp_in (pktbuf_t* buf, ipaddr_t* src_ip, ipaddr_t* dest_ip) {
    ...
    if (!udp) {
        dbg_info(DBG_UDP, "no udp for this packet");
