...
#define SYS_PLAT_TINYOS

// 系统硬件配置
// 不同网卡配置，共2块网卡
#if 1
static const char netdev0_ip[] = "192.168.5.200";
static const char netdev0_gw[] = "192.168.5.1";
static const char friend0_ip[] = "192.168.5.157";
static const char netdev0_phy_ip[] = "192.5.74.1";    // 用于收发包的真实网卡ip地址，在qemu上不需要使用
static const char netdev0_mask[] = "255.255.255.0";
static const uint8_t netdev0_hwaddr[] = { 0x00, 0x50, 0x56, 0xc0, 0x00, 0x19 };
#else
...
typedef int net_time_t;      // 时间类型
typedef os_protect_t sys_intlocker_t;

#define SYS_THREAD_INVALID          (os_task_t *)0
#define SYS_SEM_INVALID             (os_sem_t *)0
#define SYS_MUTEx_INVALID           (os_mutex_t *)0

typedef os_mutex_t * sys_mutex_t;        // 互斥锁
typedef os_task_t * sys_thread_t;        // 线程
typedef os_sem_t * sys_sem_t;            // 信号量

#define plat_strlen         strlen
#define plat_strcpy         strcpy
#define plat_strncpy        strncpy
#define plat_strcmp         strcmp
#define plat_stricmp        strncasecmp
#define plat_memset         memset
#define plat_memcpy         memcpy
#define plat_memcmp         memcmp
#define plat_sprintf        sprintf
...
int sys_sem_wait(sys_sem_t sem, int ms);
