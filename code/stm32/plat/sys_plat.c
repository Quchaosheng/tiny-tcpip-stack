...
static net_task_t task_table[NET_TASK_NR];
...
void sys_plat_init(void) {
    mblock_init(&task_mblock, task_table, sizeof(net_task_t), NET_TASK_NR, NLOCKER_NONE);
...
int sys_time_goes (net_time_t * pre) {
    // 获取当前时间
    net_time_t curr = os_tick_count();

    // 记录过去了多少毫秒
    int diff_ms = (curr - *pre) * OS_SYSTICK_MS;
...
// 计数信号量相关：由具体平台实现
sys_sem_t sys_sem_create(int init_count) {
    return os_sem_create(init_count, -1);
}
...
int sys_sem_wait(sys_sem_t sem, int ms) {
    os_err_t err;
    if (ms < 0) {
        err = os_sem_wait(sem, -1);
    } else {
        err = os_sem_wait(sem, ms);
    }
    return err < 0 ? NET_ERR_TMO: 0;
}
...
sys_intlocker_t sys_intlocker_lock(void) {
    return os_enter_protect();
}

void sys_intlocker_unlock(sys_intlocker_t locker) {
    os_leave_protect(locker);
}

sys_thread_t sys_thread_create(sys_thread_func_t entry, char * name, void* arg) {
    // 固定优先级0，栈大小8192
    sys_thread_t thread = os_task_create(name, entry, arg, 0, 3*1024);
    if (thread == OS_NULL) {
        return OS_NULL;
    }

    os_task_start(thread);
    return thread;
}
