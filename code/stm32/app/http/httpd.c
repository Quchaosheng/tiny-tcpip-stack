...
static void method_get_in(http_client_t * client, http_request_t * request) {
	...
	if (request->url[strlen(request->url) - 1] == '/') {
        sprintf(client->buffer, "%s%s%s", root_dir, request->url, "index.html");
	} else {
        sprintf(client->buffer, "%s%s", root_dir, request->url);
    }
    
    // 打开要访问的文件，一定要注意文件打开的模式
    char * name = client->buffer;
    if (client->buffer[0] == '/') {
        name++;
    }

    FILE * file = fopen(client->buffer, "rb");
    if (file == (FILE *)0) {
    ...
    const char * mime_type = mime_find(client->buffer);
    if (!mime_type) {
    ...
    size_t size = response_text(&response, client->buffer, sizeof(client->buffer));
    send(client->sock, client->buffer, size, 0);

    // 不断读取文件并写入套接字
    while ((size = fread(client->buffer, 1, sizeof(client->buffer), file)) > 0) {
        if (send(client->sock, client->buffer, size, 0) < 0) {
...
static void client_thread(void* arg) {
    ...
    // 设置超时
    struct timeval tmo;
    tmo.tv_sec = HTTPD_RCV_TMO;
    tmo.tv_usec = 0;
    setsockopt(client->sock, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tmo, sizeof(tmo));
...
static void httpd_server_thread(void* arg) {
	...
	for (;;) {
        ...
        // 当创建线程失败时，应当如何重呢？简单的重试
        while (sys_thread_create(client_thread, "httpd child", (void *)client) == SYS_THREAD_INVALID) {
            httpd_log("httpd: create client thread failed, retry\n");
            sys_sleep(5);
        }
        http_client_cnt++;
	}
...
int httpd_start (const char* dir, uint16_t port) {
    ...
    // 创建单独的线程来运行服务
    httpd = sys_thread_create(httpd_server_thread, "httpd", (void *)0);
    if (httpd == SYS_THREAD_INVALID) {
