...
static int do_send_file(tftp_req_t * req) {
    ...
    while (1) {
        // 读取文件到发送缓存
        ssize_t size = fread(tpacket->data.data, 1, tftp->block_size, file);
        if (size < 0) {
 ...
 * @brief 等待任意客户端的请求
 */
static tftp_req_t * wait_for_req(tftp_t * tftp) {
...
static void tftp_server_thread(void* arg) {
    ...
    while (1) {
        // 等待客户端发来的请求
        tftp_req_t * req = wait_for_req(&tftp);
        if (req) {
            // 创建工作线程处理该请求
            if (sys_thread_create(tftp_working_thread, "tftpd child", req) == SYS_THREAD_INVALID) {
                free(req);
            }
        }
    }
...
net_err_t tftpd_start (const char* dir, uint16_t port) {
    ...
    // 创建单独的线程来运行服务
    sys_thread_t thread = sys_thread_create(tftp_server_thread, "tftpd", (void *)0);
