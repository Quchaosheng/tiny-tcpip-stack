...
static void show_client_info (char * title, http_client_t * client) {
    ...
    printf("httpd: %s ip: %s, port: %d\n",  title, ipbuf, ntohs(client->addr.sin_port));
}
...
static void http_404_not_found (http_client_t * client) {
	...
	static const char response[] =
		...
		"\r\n"
		"<htm><title>File not found</title><body><h1>Not Found</h1><p>File not found</p></body></htm>";
...
static void http_400_bad_request (http_client_t * client) {
	...
	static const char response[] =
		...
		"Content-Type: text/htm\r\n"
		"\r\n"
		"<htm><title>Bad Request</title><body>Bad Request from your browser</body></htm>";
	send(client->sock, response, sizeof(response), 0);
}
...
static void http_501_not_implemented (http_client_t * client) {
	...
	static const char response[] =
		...
		"Content-Type: text/htm\r\n"
		"\r\n"
		"<htm><title>Not Implementedt</title><body>Unsupport method from you frowser</body></htm>";
	send(client->sock, response, sizeof(response), 0);
}
...
static void http_403_forbidden (http_client_t * client) {
	...
	static const char response[] =
		...
		"Content-Type: text/htm\r\n"
		"\r\n"
		"<htm><title>Forbidden</title><body>You don't have acces to this content</body></htm>";
	send(client->sock, response, sizeof(response), 0);
}
...
static void method_get_in(http_client_t * client, http_request_t * request) {
	...
	if (request->url[strlen(request->url) - 1] == '/') {
        //sprintf(buf, "%s%s%s", root_dir, request->url, "index.htm");
        sprintf(buf, "%s%s%s", root_dir, request->url, "index.htm");
	} else {
 ...
 * @brief 处理具体客户端连接的线程
 */
static void process_client(http_client_t * client) {
    // 开启Keepalive
    int keepalive = 1;
    int keepidle = 5;           // 空间一段时间后
    int keepinterval = 1;       // 时间多长时间
    int keepcount = 10;         // 重发多少次keepalive包
    setsockopt(client->sock, SOL_SOCKET, SO_KEEPALIVE, (void *)&keepalive , sizeof(keepalive ));
    setsockopt(client->sock, SOL_TCP, TCP_KEEPIDLE, (void*)&keepidle , sizeof(keepidle ));
    setsockopt(client->sock, SOL_TCP, TCP_KEEPINTVL, (void *)&keepinterval , sizeof(keepinterval ));
    setsockopt(client->sock, SOL_TCP, TCP_KEEPCNT, (void *)&keepcount , sizeof(keepcount ));
