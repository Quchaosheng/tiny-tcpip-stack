 ...
 * 网络设备打开
 */
static net_err_t rtl8139_open(struct _netif_t* p_netif, void* ops_data) {    
    static int idx = 0;
...
static net_err_t rtl8139_xmit (struct _netif_t* netif) {
    ...
    if (priv->tx.free_count <= 0) {
        dbg_warning(DBG_NETIF, "tx free_cnt == 0");
        return NET_ERR_OK;
    } 
    ...
    if (isr & INT_ROK) {
        ...
        while ((inb(priv->ioaddr + CR) & CR_BUFE) == 0) {
            ...
            if (hdr->FAE || hdr->CRC || hdr->LONG || hdr->RUNT || hdr->ISE) {
                dbg_warning(DBG_NETIF, "%s: rcv pkt error: %x", netif->name);
                // 重新对接收进行设置，重启。还是忽略这个包，还是忽略吧
                rtl8139_setup_rx();
            } else if ((pkt_size < 0) || (pkt_size >= R8139DN_MAX_ETH_SIZE)) {
                dbg_warning(DBG_NETIF, "%s: size error: %x", netif->name);
                // 重新对接收进行设置，重启。还是忽略这个包，还是忽略吧
                rtl8139_setup_rx();
            }else {
                // 分配包缓存空间
                ...
                if (buf) {
                    ...
                    // 包不够则丢弃
                    dbg_warning(DBG_NETIF, "no buffer");
                }
